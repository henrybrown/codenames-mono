 ...
 *
 * Turns `LLMConfig` into a usable LLM client. This is the only sub-module
 * that knows about specific providers (OpenAI, Anthropic, Gemini, Ollama).
 * Everything below this layer (pipeline, player, move) treats the LLM as
 * an opaque service.
 */
...
export const createModels = (logger: AppLogger) => (deps: ModelsDependencies) => {
  const llm = createLLMService(deps.config, logger);
  return {
    llm,
  };
};
